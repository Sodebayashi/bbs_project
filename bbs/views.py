...
class CreateView(LoginRequiredMixin, generic.edit.CreateView):
    ...
    def form_valid(self, form): # バリデーション通過時に呼び出し
        form.instance.author = self.request.user # authorにログインユーザーを渡す
        return super(CreateView, self).form_valid(form) # 親クラスのメソッド呼び出し
...
class UpdateView(LoginRequiredMixin, generic.edit.UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        obj = self.get_object() # 編集対象のオブジェクトを取得
        if obj.author != self.request.user: # 投稿者 != ログインユーザーなら403エラー発生
            raise PermissionDenied("編集権限がありません") # 403エラーページを出力
        # UpdataViewのdispatchメソッドを実行し返す
